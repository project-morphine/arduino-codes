...
void loop() {
 ...
 if (gZArr->size() >= 100) {
  if (Firebase.ready() && signupOK) {
    Firebase.RTDB.setArray(&fbdo, ("ax/array" + (String)batchNumber) , aXArr);
    Firebase.RTDB.setArray(&fbdo, ("ay/array" + (String)batchNumber), aYArr);
    Firebase.RTDB.setArray(&fbdo, ("az/array" + (String)batchNumber), aZArr);
    Firebase.RTDB.setArray(&fbdo, ("gx/array" + (String)batchNumber), gXArr);
    Firebase.RTDB.setArray(&fbdo, ("gy/array" + (String)batchNumber), gYArr);
    if (Firebase.RTDB.setArray(&fbdo, ("gz/array" + (String)batchNumber), gZArr)) {
      Serial.println("Passed");
    } else {
  ...
  if (batchNumber >= 4) {
    batchNumber = 0;
    if (Firebase.ready() && signupOK) Firebase.RTDB.setInt(&fbdo, "done", 1);
  }
 }
 delay(50);
}
